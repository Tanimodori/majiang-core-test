declare module '@kobalab/majiang-core' {
  declare namespace Util {
    ...
    /**
     * ``shoupai`` に1枚加えるとシャンテン数の進む牌の配列を返す。
     * ``f_xiangting`` で指定された関数をシャンテン数計算の際に使用する。
     * @param shoupai 手牌
     * @param f_xiangting 指定されたシャンテン数計算関数。
     * @returns 牌の配列。返り値には赤牌は含まない。``shoupai`` がツモると多牌になる場合は ``null`` を返す。
     */
    function tingpai(shoupai: Shoupai, f_xiangting: XiangtingFunction = xiangting): Pai[] | null;
  }
}
