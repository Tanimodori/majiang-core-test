...
suite('Majiang.Shan', () => {
  ...
  suite('static zhenbaopai(p)', () => {
    test('一萬 → 二萬', () => assert.equal(Majiang.Shan.zhenbaopai('m1'), 'm2'));
    test('九萬 → 一萬', () => assert.equal(Majiang.Shan.zhenbaopai('m9'), 'm1'));
    test('赤五萬 → 六萬', () => assert.equal(Majiang.Shan.zhenbaopai('m0'), 'm6'));
    test('一筒 → 二筒', () => assert.equal(Majiang.Shan.zhenbaopai('p1'), 'p2'));
    test('九筒 → 一筒', () => assert.equal(Majiang.Shan.zhenbaopai('p9'), 'p1'));
    test('赤五筒 → 六筒', () => assert.equal(Majiang.Shan.zhenbaopai('p0'), 'p6'));
    test('一索 → 二索', () => assert.equal(Majiang.Shan.zhenbaopai('s1'), 's2'));
    test('九索 → 一索', () => assert.equal(Majiang.Shan.zhenbaopai('s9'), 's1'));
    test('赤五索 → 六索', () => assert.equal(Majiang.Shan.zhenbaopai('s0'), 's6'));
    ...
    test('不正な牌 → エラー', () => assert.throws(() => Majiang.Shan.zhenbaopai('z0')));
  });
  ...
  suite('constructor(rule)', () => {
    test('赤牌なしでインスタンスが生成できること', () => {
      ...
      assert.equal(new Majiang.Shan({ 赤牌: { m: 0, p: 0, s: 0 } })._pai.concat().sort().join(), pai);
    });
    test('赤牌ありでインスタンスが生成できること', () => {
      const pai =
        ...
        'z1,z1,z1,z1,z2,z2,z2,z2,z3,z3,z3,z3,z4,z4,z4,z4,' +
        'z5,z5,z5,z5,z6,z6,z6,z6,z7,z7,z7,z7';
      assert.equal(new Majiang.Shan({ 赤牌: { m: 1, p: 2, s: 3 } })._pai.concat().sort().join(), pai);
    });
  });
  ...
  suite('get baopai()', () => {
    test('牌山生成直後のドラは1枚', () => assert.equal(Shan().baopai.length, 1));
  });

  suite('get fubaopai()', () => {
    test('牌山生成直後は null を返す', () => assert.ifError(Shan().fubaopai));
    test('牌山固定後は裏ドラを返す', () => assert.equal(Shan().close().fubaopai.length, 1));
  ...
  suite('zimo()', () => {
    ...
    test('牌山固定後はツモれないこと', () => assert.throws(() => Shan().close().zimo()));
  });
