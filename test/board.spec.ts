...
suite('Majiang.Board', () => {
  ...
  suite('constructor(kaiju)', () => {
    ...
    test('タイトルが設定されること', () => assert.equal(board.title, 'タイトル'));
    test('対局者情報が設定されること', () => assert.deepEqual(board.player, ['私', '下家', '対面', '上家']));
    test('起家が設定されること', () => assert.equal(board.qijia, 1));
    test('パラメータなしでもインスタンスが生成できること', () => assert.ok(new Majiang.Board()));
  });
  ...
  suite('qipai(qipai)', () => {
    ...
    test('ドラが設定されること', () => assert.equal(board.shan.baopai[0], 'm1'));
    test('持ち点が設定されること', () => assert.equal(board.defen[0], 20000));
    test('手牌が設定されること', () => assert.equal(board.shoupai[1].toString(), 'm123p456s789z1234'));
    test('捨て牌が初期化されること', () =>
      assert.equal(
  ...
  suite('zimo(zimo)', () => {
    ...
    test('手牌にツモ牌が加えられること', () => assert.equal(board.shoupai[0].get_dapai().pop(), 'm1_'));
    test('伏せ牌をツモれること', () => {
      const board = init_board();
      board.zimo({ l: 0, p: '' });
  ...
  suite('dapai(dapai)', () => {
    ...
    board.zimo({ l: 0, p: 'm1' });
    board.dapai({ l: 0, p: 'm1_' });
    test('手牌から打牌が切り出されること', () => assert.ifError(board.shoupai[0].get_dapai()));
    test('捨て牌に加えられること', () => assert.equal(board.he[0]._pai[0], 'm1_'));
    test('少牌となる打牌ができること', () => {
      const board = init_board();
  ...
  suite('fulou(fulou)', () => {
    ...
    test('河から副露牌が拾われること', () => assert.equal(board.he[0]._pai[0], 'm1_='));
    test('手番が移動すること', () => assert.equal(board.lunban, 2));
    test('手牌が副露されること', () => assert.equal(board.shoupai[2]._fulou[0], 'm111='));
  ...
  suite('gang(gang)', () => {
    ...
    test('手牌が副露されること', () => assert.equal(board.shoupai[0]._fulou[0], 'm1111'));
    test('少牌となるカンができること', () => {
      const board = init_board();
      board.gang({ l: 0, m: 'm1111' });
    });
  ...
  suite('hule(hule)', () => {
    ...
    test('裏ドラを参照できること', () => assert.equal(board.shan.fubaopai[0], 's9'));
