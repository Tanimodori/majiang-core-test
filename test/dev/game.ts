...
function make_reply(l, log) {
  ...
  for (const data of log) {
    ...
    else if (data.dapai) reply.push(l == data.dapai.l ? { dapai: data.dapai.p } : {});
    else if (data.fulou) reply.push(l == data.fulou.l ? { fulou: data.fulou.m } : {});
    else if (data.gang) reply.push(l == data.gang.l ? { gang: data.gang.m } : {});
    else if (data.pingju) {
...
export default class Game extends Majiang.Game {
  ...
  qipai() {
    ...
    for (let l = 0; l < 4; l++) {
      const id = (this._model.qijia + this._model.jushu + l) % 4;
