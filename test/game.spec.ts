...
suite('Majiang.Game', () => {
  ...
  suite('constructor(players, callback, rule)', () => {
    ...
    test('対局者名が設定されていること', () => assert.deepEqual(game._model.player, ['私', '下家', '対面', '上家']));
    test('局数が初期化されていること', () => {
    ...
    test('持ち点が初期化されていること', () => assert.deepEqual(game._model.defen, [25000, 25000, 25000, 25000]));
    test('持ち点が変更可能なこと', () => {
  ...
  suite('kaiju(qijia)', () => {
    ...
    test('起家を乱数で設定できること', () => {
      game.stop();
      game.kaiju();
      assert.ok(game._model.qijia == 0 || game._model.qijia == 1 || game._model.qijia == 2 || game._model.qijia == 3);
    });
  });
  ...
  suite('qipai(shan)', () => {
    ...
    test('配牌されること', () => {
      for (let l = 0; l < 4; l++) {
        assert.equal(game.model.shoupai[l].toString().replace(/[mpsz]/g, '').length, 13);
      }
    });
    test('河が初期化されること', () => {
      for (let l = 0; l < 4; l++) {
  ...
  suite('zimo()', () => {
    ...
    test('牌山からツモられること', () => assert.equal(game.model.shan.paishu, 69));
    test('手牌にツモ牌が加えられること', () => assert.ok(game.model.shoupai[0].get_dapai()));
  ...
  suite('dapai(dapai)', () => {
    ...
    test('河に打牌されること', () => assert.equal(game.model.he[0]._pai[0], dapai));
    ...
    test('一発なし', () => {
      const game = init_game({ rule: Majiang.rule({ 一発あり: false }), shoupai: ['_', '', '', ''] });
      game._diyizimo = false;
      game.zimo();
      game.dapai('m1*');
      assert.equal(game._lizhi[game.model.lunban], 1);
  ...
  suite('fulou(fulou)', () => {
    ...
    test('手番が更新されること(上家からのチー)', () => assert.equal(game.model.lunban, 1));
    test('手牌が副露されること', () => assert.equal(game.model.shoupai[1]._fulou[0], 'm12-3'));
  ...
  suite('gangzimo()', () => {
    ...
    test('カンドラ後乗せではない場合、大明槓も即座に開槓されること', () => {
      const game = init_game({ rule: Majiang.rule({ カンドラ後乗せ: false }), shoupai: ['_', '', '_', ''] });
      game.zimo();
      game.dapai('s3');
      game.fulou('s3333=');
      game.gangzimo();
  ...
  suite('hule()', () => {
    ...
    test('通知のタイミングを変更できること', (done) => {
      const game = init_game({ shoupai: ['m123p456s789z1122', '', '', ''], zimo: ['z2'] });
      game.wait = 20;
      game.zimo();
      MSG = [];
      game._sync = false;
    ...
    test('ダブル立直', () => {
      ...
      game._hule = [0];
      game.hule();
      assert.ok(last_paipu(game).hule.hupai.find((h) => h.name == 'ダブル立直'));
    });
    test('槍槓', () => {
      const game = init_game({ shoupai: ['_________m1,m111=', '_', 'm23p456s789z11222', ''] });
      game.zimo();
      game.gang('m111=1');
      game._hule = [2];
      game.hule();
    ...
    test('嶺上開花', () => {
      const game = init_game({ shoupai: ['m123p456s78z11,m111=', '', '', ''], zimo: ['m4'], gangzimo: ['s9'] });
      game.zimo();
      game.gang('m111=1');
      game.gangzimo();
      game.hule();
    ...
    test('地和', () => {
      const game = init_game({ shoupai: ['_', 'm123p456s789z1122', '', ''], zimo: ['m1', 'z2'] });
      game.zimo();
      game.dapai('m1_');
      game.zimo();
      game.hule();
    ...
    test('槍槓でダブロン', () => {
      const game = init_game({ shoupai: ['__________,m111=', 'm23p456s789z11122', 'm23p789s456z33344', ''] });
      game.zimo();
      game.gang('m111=1');
      game._hule = [1, 2];
      game.hule();
    ...
    test('親の和了は連荘', () => {
      const game = init_game({ shoupai: ['m123p456s789z1122', '', '', ''], zimo: ['z1'] });
      game.zimo();
      game.hule();
      assert.ok(game._lianzhuang);
    });
    test('ダブロンは親の和了があれば連荘', () => {
      const game = init_game({ shoupai: ['m23p456s789z11122', '_', 'm23p789s546z33344', ''], zimo: ['m2', 'm1'] });
      game.zimo();
      game.dapai('m2');
      game.zimo();
      game.dapai('m1');
  ...
  suite('pingju(name, shoupai)', () => {
    ...
    test('全員テンパイ', () => {
      const game = init_game({
        ...
        shoupai: ['m22p12366s406789', 'm55p40s123,z111-,p678-', 'm67p678s22,s56-7,p444-', 'm12345p33s333,m406-'],
      });
      game.pingju();
    ...
    test('全員ノーテン', () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false }),
        shoupai: ['m40789p4667s8z577', 'm99p12306z277,m345-', 'm3p1234689z55,s7-89', 'm2233467p234555'],
      });
      game.pingju();
      assert.equal(last_paipu(game).pingju.name, '荒牌平局');
    ...
    test('2人テンパイ', () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false }),
        shoupai: ['m22p12366s406789', 'm99p12306z277,m345-', 'm3p1234689z55,s7-89', 'm12345p33s333,m406-'],
      });
      game.pingju();
      assert.equal(last_paipu(game).pingju.name, '荒牌平局');
    ...
    test('形式テンパイとならない牌姿', () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false }),
        shoupai: ['m123p456s789z1111', 'm99p12306z277,m345-', 'm3p1234689z55,s7-89', 'm12345p33s333,m406-'],
      });
      game.pingju();
      assert.equal(last_paipu(game).pingju.name, '荒牌平局');
    ...
    test('テンパイ連荘', () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false }),
        shoupai: ['m22p12366s406789', 'm99p12306z277,m345-', 'm3p1234689z55,s7-89', 'm2233467p234555'],
      });
      game.pingju();
      assert.ok(game._lianzhuang);
    });
    ...
    test('ノーテン罰なしのルールの場合、' + 'リーチ者と親以外は手牌を開かないこと', () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false, ノーテン罰あり: false }),
        shoupai: ['m567999s4466777', 'm05p123s56z333*,s8888', 'm11p789s06,z555-,p406-', ''],
      });
      game.pingju();
      assert.deepEqual(last_paipu(game).pingju.shoupai, ['m567999s4466777', 'm05p123s56z333*,s8888', '', '']);
    });
    test('ノーテン罰なしで和了連荘のルールの場合、' + 'リーチ者以外は手牌を開かないこと', () => {
      const game = init_game({
        rule: Majiang.rule({ 流し満貫あり: false, ノーテン罰あり: false, 連荘方式: 1 }),
        shoupai: ['m567999s4466777', 'm05p123s56z333*,s8888', 'm11p789s06,z555-,p406-', ''],
      });
      game.pingju();
      assert.deepEqual(last_paipu(game).pingju.shoupai, ['', 'm05p123s56z333*,s8888', '', '']);
    });
  });
  ...
  suite('last()', () => {
    ...
    test('東風戦は東四局で終局すること', () => {
      const game = init_game({ rule: Majiang.rule({ 場数: 1 }), defen: [10000, 20000, 30000, 40000] });
      game.model.zhuangfeng = 0;
      game.model.jushu = 3;
      game.last();
    ...
    test('東南戦は南四局で終局すること', () => {
      const game = init_game({ rule: Majiang.rule({ 場数: 2 }), defen: [10000, 20000, 30000, 40000] });
    ...
    test('一荘戦は北四局で終局すること', () => {
      const game = init_game({ rule: Majiang.rule({ 場数: 4 }), defen: [10000, 20000, 30000, 40000] });
    ...
    test('トビ終了なし', () => {
      const game = init_game({ rule: Majiang.rule({ トビ終了あり: false }), defen: [50100, 30000, 20000, -100] });
      game.last();
      assert.equal(game._status, 'qipai');
    });
    test('オーラス止め(東風戦)', () => {
      const game = init_game({ rule: Majiang.rule({ 場数: 1 }), defen: [40000, 30000, 20000, 10000] });
      game.model.zhuangfeng = 0;
      game.model.jushu = 3;
      game._lianzhuang = true;
      game.last();
    ...
    test('オーラス止めなし', () => {
      const game = init_game({ rule: Majiang.rule({ オーラス止めあり: false }), defen: [40000, 30000, 20000, 10000] });
      game.model.zhuangfeng = 1;
      game.model.jushu = 3;
      game._lianzhuang = true;
      game.last();
  ...
  suite('jieju()', () => {
    ...
    test('終局時に残った供託リーチ棒はトップの得点に加算', () => {
      const game = init_game({ qijia: 3, defen: [9000, 19000, 29000, 40000], lizhibang: 3 });
      game.jieju();
    ...
    test('順位点を変更できること', () => {
      const game = init_game({ rule: Majiang.rule({ 順位点: ['30', '10', '-10', '-30'] }), qijia: 2 });
      game.jieju();
      assert.deepEqual(game._paipu.rank, [3, 4, 1, 2]);
  ...
  suite('reply_zimo()', () => {
    ...
    test('リーチ', () => {
      const game = init_game({ shoupai: ['m123p456s789z1122', '', '', ''], zimo: ['m1'] });
      set_reply(game, [{ dapai: 'm1_*' }, {}, {}, {}]);
      game.zimo();
      game.next();
      assert.equal(last_paipu(game).dapai.p, 'm1_*');
    ...
    test('途中流局なしの場合は九種九牌にできないこと', () => {
      const game = init_game({ rule: Majiang.rule({ 途中流局あり: false }), shoupai: ['m123459z1234567', '', '', ''] });
      set_reply(game, [{ daopai: '-' }, {}, {}, {}]);
      game.zimo();
      game.next();
      assert.ok(last_paipu(game).dapai);
    ...
    test('ツモ和了(不正応答)', () => {
      const game = init_game({ shoupai: ['m123p456s789z1122', '', '', ''], zimo: ['z3'] });
      set_reply(game, [{ hule: '-' }, {}, {}, {}]);
      game.zimo();
      game.next();
      assert.ok(last_paipu(game).dapai);
    ...
    test('カン', () => {
      const game = init_game({ shoupai: ['m123p456z1122,s888+', '', '', ''], zimo: ['s8'] });
    ...
    test('カン(不正応答)', () => {
      const game = init_game({ shoupai: ['m123p456z1122,s888+', '', '', ''], zimo: ['s7'] });
      set_reply(game, [{ gang: 's888+8' }, {}, {}, {}]);
      game.zimo();
      game.next();
      assert.ok(last_paipu(game).dapai);
  ...
  suite('reply_dapai()', () => {
    ...
    test('ダブロン', () => {
      const game = init_game({ shoupai: ['_', 'm23446p45688s345', 'm34s33,s444-,s666+,p406-', ''] });
      game.zimo();
    ...
    test('三家和', () => {
      const game = init_game({
        shoupai: ['_', 'm23446p45688s345', 'm34s33,s444-,s666+,p406-', 'm23467s88,s222+,z666='],
      });
      game.zimo();
    ...
    test('リーチ成立', () => {
      const game = init_game({ shoupai: ['m55688p234567s06', '', '', ''], qijia: 0, zimo: ['s7'] });
      game.zimo();
      game.dapai('m5*');
      game.next();
    ...
    test('リーチ不成立', () => {
      const game = init_game({ shoupai: ['m55688p234567s06', 'm23446p45688s345', '', ''], qijia: 0, zimo: ['s7'] });
      game.zimo();
      set_reply(game, [{}, { hule: '-' }, {}, {}]);
      game.dapai('m5*');
      game.next();
    ...
    test('四家立直', () => {
      const game = init_game({
        shoupai: ['m11156p5688s2346', 'm2227p11340s2356', 'm2346789p345699', 'm34056p4678s3456'],
        qijia: 0,
        zimo: ['p4', 's1', 'm7', 's6'],
      });
    ...
    test('途中流局なしの場合は四風連打とならず、第一ツモ巡が終了すること', () => {
      const game = init_game({ rule: Majiang.rule({ 途中流局あり: false }), shoupai: ['_', '_', '_', '_'] });
      for (let l = 0; l < 4; l++) {
        game.zimo();
        game.dapai('z1');
      }
    ...
    test('流局', () => {
      ...
      game.next();
      assert.equal(last_paipu(game).pingju.name, '荒牌平局');
      assert.deepEqual(last_paipu(game).pingju.shoupai, ['', 'm222p11340s12356', 'm23467789p34599', '']);
    });
    test('カン', () => {
    ...
    test('ポンとチーの競合はポンを優先', () => {
      const game = init_game({ shoupai: ['_', 'm23567p456s889z11', 'm11789p123s11289', ''] });
      game.zimo();
  ...
  suite('reply_gang()', () => {
    test('応答なし', () => {
      const game = init_game({ shoupai: ['m45p456s11789,m111+', '', '', ''], zimo: ['m1'] });
      game.zimo();
      game.gang('m111+1');
      game.next();
      assert.ok(last_paipu(game).gangzimo);
    ...
    test('ロン和了(槍槓)', () => {
      const game = init_game({ shoupai: ['m45p456s11789,m111+', '', '', 'm23456p123s67899'], zimo: ['m1'] });
      game.zimo();
      set_reply(game, [{}, {}, {}, { hule: '-' }]);
      game.gang('m111+1');
      game.next();
    ...
    test('ロン和了(不正応答)', () => {
      const game = init_game({ shoupai: ['m45p456s11789,m111+', '', '', 'm33456p123s67899'], zimo: ['m1'] });
      game.zimo();
      set_reply(game, [{}, {}, {}, { hule: '-' }]);
      game.gang('m111+1');
      game.next();
    ...
    test('暗槓は槍槓できない', () => {
      const game = init_game({ shoupai: ['m11145p456s11789', '', '', 'm23456p123s67899'], zimo: ['m1'] });
      game.zimo();
      set_reply(game, [{}, {}, {}, { hule: '-' }]);
      game.gang('m1111');
      game.next();
    ...
    test('ダブロン', () => {
      const game = init_game({
        shoupai: ['m11p222s88,z666=,m505-', 'm23446p45688s345', 'm34s33,s444-,s666+,p406-', ''],
        zimo: ['m5'],
      });
      game.zimo();
      set_reply(game, [{}, { hule: '-' }, { hule: '-' }, {}]);
  ...
  suite('get_dapai()', () => {
    test('現在の手番の可能な打牌を返すこと', () => {
      const game = init_game({ shoupai: ['m123,z111+,z222=,z333-', '', '', ''], zimo: ['m1'] });
      game.zimo();
  ...
  suite('get_gang_mianzi(l)', () => {
    ...
    test('ハイテイ牌は暗槓もしくは加槓できないこと', () => {
      const game = init_game({ shoupai: ['m1111p4567s78,z111='], zimo: ['z1'] });
      game.zimo();
      while (game.model.shan.paishu) game.model.shan.zimo();
  ...
  suite('allow_lizhi(p)', () => {
    test('指定された打牌でリーチ可能な場合、真を返す', () => {
      const game = init_game({ shoupai: ['m123p456s789z1123', '', '', ''], zimo: ['z2'] });
      game.zimo();
  ...
  suite('allow_hule(l)', () => {
    test('ツモ和了', () => {
      const game = init_game({ shoupai: ['m123p456s789z3344', '', '', ''], zimo: ['z4'] });
      game.zimo();
      assert.ok(game.allow_hule());
    });
    test('リーチ・ツモ', () => {
      const game = init_game({ shoupai: ['m123p456s789z4*,z333=', '', '', ''], zimo: ['z4'] });
      game.zimo();
      assert.ok(game.allow_hule());
    });
    test('嶺上開花', () => {
      const game = init_game({ shoupai: ['_', '', 'm123p456s789z3334', ''], gangzimo: ['z4'] });
      game.zimo();
      game.dapai('z3');
      game.fulou('z3333=');
      game.gangzimo();
    ...
    test('場風のみ・ツモ', () => {
      const game = init_game({ shoupai: ['_', 'm123p456s789z4,z111=', '', ''], zimo: ['m1', 'z4'] });
      game.zimo();
      game.dapai('m1');
      game.zimo();
      assert.ok(game.allow_hule());
    ...
    test('自風のみ・ツモ', () => {
      const game = init_game({ shoupai: ['_', 'm123p456s789z4,z222=', '', ''], zimo: ['m1', 'z4'] });
      game.zimo();
      game.dapai('m1');
      game.zimo();
      assert.ok(game.allow_hule());
    ...
    test('槍槓', () => {
      const game = init_game({ shoupai: ['m1234p567s789,m111=', '', 'm23p123567s12377', ''] });
      game.zimo();
      game.gang('m111=1');
      assert.ok(game.allow_hule(2));
    });
    test('ハイテイ・ロン', () => {
      const game = init_game({ shoupai: ['_', '', '', 'm123p456s789z4,z333='] });
      game.zimo();
      while (game.model.shan.paishu) game.model.shan.zimo();
      game.dapai('z4');
    ...
    test('フリテンはロン和了できないこと', () => {
      const game = init_game({ shoupai: ['m123p456s789z3344', '', '', ''], zimo: ['z4', 'z3'] });
      game.zimo();
      game.dapai('z4');
      game.zimo();
      game.dapai('z3');
  ...
  suite('static get_dapai(rule, shoupai)', () => {
    ...
    test('現物以外の喰い替えあり', () =>
      assert.deepEqual(Majiang.Game.get_dapai(Majiang.rule({ 喰い替え許可レベル: 1 }), shoupai), [
        'm8',
        'p5',
        'p6',
        'p7',
    ...
    test('現物喰い替えもあり', () =>
      assert.deepEqual(Majiang.Game.get_dapai(Majiang.rule({ 喰い替え許可レベル: 2 }), shoupai), [
        'm5',
        'm8',
        'p5',
        'p6',
  ...
  suite('static get_chi_mianzi(rule, shoupai, p, paishu)', () => {
    ...
    test('現物以外の喰い替えあり', () => {
      ...
      assert.deepEqual(Majiang.Game.get_chi_mianzi(rule, shoupai1, 'm1-', 1), ['m1-23']);
      assert.deepEqual(Majiang.Game.get_chi_mianzi(rule, shoupai2, 'm1-', 1), []);
    });
    test('現物喰い替えもあり', () => {
      const rule = Majiang.rule({ 喰い替え許可レベル: 2 });
      assert.deepEqual(Majiang.Game.get_chi_mianzi(rule, shoupai1, 'm1-', 1), ['m1-23']);
      assert.deepEqual(Majiang.Game.get_chi_mianzi(rule, shoupai2, 'm1-', 1), ['m1-23']);
    });
    test('ハイテイは鳴けない', () => {
      assert.deepEqual(Majiang.Game.get_chi_mianzi(Majiang.rule({ 喰い替え許可レベル: 2 }), shoupai1, 'm1-', 0), []);
    });
    test('ツモした状態でチーできない', () => {
      assert.ifError(
        Majiang.Game.get_chi_mianzi(Majiang.rule(), Majiang.Shoupai.fromString('m123p456s12789z123'), 's3-', 1),
      );
    });
  });
  ...
  suite('static get_peng_mianzi(rule, shoupai, p, paishu)', () => {
    ...
    test('ハイテイは鳴けない', () =>
      assert.deepEqual(Majiang.Game.get_peng_mianzi(Majiang.rule({ 喰い替え許可レベル: 0 }), shoupai, 'm1+', 0), []));
    test('ツモした状態でポンできない', () => {
      assert.ifError(
        Majiang.Game.get_peng_mianzi(Majiang.rule(), Majiang.Shoupai.fromString('m123p456s11789z123'), 's1-', 1),
      );
    });
  });
  ...
  suite('static get_gang_mianzi(rule, shoupai, p, paishu)', () => {
    ...
    test('リーチ後の暗槓なし', () => {
      ...
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai2, null, 1), []);
      ...
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai5, null, 1), []);
    });
    test('リーチ後の牌姿の変わる暗槓なし', () => {
      const rule = Majiang.rule({ リーチ後暗槓許可レベル: 1 });
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai1, null, 1), ['z1111']);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai2, null, 1), []);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai3, null, 1), []);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai4, null, 1), []);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai5, null, 1), []);
    ...
    test('リーチ後の待ちの変わる暗槓なし', () => {
      ...
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai1, null, 1), ['z1111']);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai2, null, 1), []);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai3, null, 1), ['s3333']);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai4, null, 1), ['s1111']);
      assert.deepEqual(Majiang.Game.get_gang_mianzi(rule, shoupai5, null, 1), []);
    });
    test('ハイテイはカンできない', () => {
      const rule = Majiang.rule();
      assert.deepEqual(
        Majiang.Game.get_gang_mianzi(rule, Majiang.Shoupai.fromString('m1112p456s789z111z1'), null, 0),
        [],
      );
      assert.deepEqual(
        Majiang.Game.get_gang_mianzi(rule, Majiang.Shoupai.fromString('m1112p456s789z111'), 'z1=', 0),
        [],
      );
    });
  });
